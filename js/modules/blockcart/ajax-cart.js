		...
		$.ajax({
			...
			success: function(jsonData,textStatus,jqXHR)
			{
				...
				if($('body').is('#index')) {
					...
					$(callerElement).parent().parent().parent().find('.added').show("slow");
					setTimeout(function(){$(callerElement).parent().parent().parent().find('.added').hide("slow");},5000);
				} else {
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('#cart_block_product_'+ domIdProduct ).length == 0)
				{
					...
					content += '<span class="product_image"><img src="' + this.id_image + '" alt="' + this.name +'"></span>';
